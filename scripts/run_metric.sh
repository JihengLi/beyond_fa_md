...
dwi_mha_files=$(find /input/ -type f -name "*.mha")
...
for dwi_mha_file in $dwi_mha_files; do
    ...
    output_dir="/tmp/tractseg_md_output"
    mkdir -p $output_dir
    ...
    # Make json with json["md"]["mean"] = mean of md in bundle
    ...
    for roi in $roi_list; do
        ...
        if [ $mask_sum -eq 0 ]; then
            ...
            mean_metric=$(fslstats $md_dir/$metric.nii.gz -k $roi -m)
            echo "$bundle_name,$mean_metric" >> ${output_dir}/tensor_metrics.json
        fi
    done
    # scil_volume_stats_in_ROI.py --metrics_dir ${md_dir} $roi_list > ${output_dir}/tensor_metrics.json

    # Extract specified metric to JSON
    echo "Extracting $metric metrics to $output_dir..."
    python extract_metric.py ${output_dir}/tensor_metrics.json $output_dir/md.json

    # Save the final metric.json to output directory
    echo "$metric metrics saved to $output_name!"
    mv $output_dir/md.json $output_name
